      ...
      <section className="py-12 bg-gradient-to-b from-emerald-50/50 to-white">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <div className="inline-block px-4 py-2 bg-emerald-100/50 rounded-full animate-pulse">
            <span className="text-emerald-600 text-sm font-medium">Loading New Arrivals...</span>
          </div>
        </div>
      </section>
    );
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Fresh products just added</p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 md:gap-6">
          {products.slice(0, 4).map((product) => {
            ...
            return (
              <div
                key={product.id}
                className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100/80 hover:border-emerald-200/80"
              >
                ...
                <Link to={`/products/${product.id}`}>
                  <div className="bg-gradient-to-b from-gray-50 to-gray-100 p-6 flex justify-center items-center h-48 relative overflow-hidden cursor-pointer">
                    ...
                    <span className="absolute top-3 right-3 bg-emerald-500 text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-sm shadow-emerald-200 flex items-center gap-1">
                      <Sparkles size={10} />
                      New
                    </span>
                  </div>
