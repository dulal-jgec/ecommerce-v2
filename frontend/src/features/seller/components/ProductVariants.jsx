...
const ProductVariants = ({ variants, setVariants }) => {
  ...
  const addVariant = () => {
    if (newVariant.name && newVariant.price && newVariant.stock) {
      setVariants([
        ...variants,
        { ...newVariant, id: Date.now(), price: parseFloat(newVariant.price), stock: parseInt(newVariant.stock) }
      ]);
  ...
  return (
    <div className="space-y-6">
            ...
            <tbody className="divide-y divide-gray-50">
              {variants.map((variant) => (
                <tr key={variant.id} className="hover:bg-gray-50/50 transition">
                  <td className="px-4 py-3 font-medium text-sm text-gray-800">{variant.name}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">₹{variant.price}</td>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-700">
        ...
        <p className="text-blue-600 mt-1">Add size, color, weight, or other options for your product.</p>
      </div>
    </div>
  );
};
