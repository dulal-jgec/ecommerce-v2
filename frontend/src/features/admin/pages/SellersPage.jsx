...
import { getPendingSellers, approveSeller, rejectSeller } from '../services/adminService';
...
const SellersPage = () => {
  ...
  const stats = [
    { label: 'Total Sellers', value: sellers.length, color: 'bg-indigo-50 text-indigo-700', icon: Store },
    { label: 'Pending', value: sellers.filter(s => s.status === 'PENDING' || !s.status).length, color: 'bg-yellow-50 text-yellow-700', icon: Clock },
    { label: 'Approved', value: sellers.filter(s => s.status === 'APPROVED').length, color: 'bg-emerald-50 text-emerald-700', icon: CheckCircle },
    { label: 'Rejected', value: sellers.filter(s => s.status === 'REJECTED').length, color: 'bg-red-50 text-red-700', icon: XCircle },
  ];

  const filteredSellers = sellers.filter(seller => {
    const matchesSearch = 
      (seller.storeName || seller.shopName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      'APPROVED': { icon: <CheckCircle size={14} />, color: 'bg-emerald-50 text-emerald-700', label: 'Approved' },
      'PENDING': { icon: <Clock size={14} />, color: 'bg-yellow-50 text-yellow-700', label: 'Pending' },
      'REJECTED': { icon: <XCircle size={14} />, color: 'bg-red-50 text-red-700', label: 'Rejected' },
    };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Manage all sellers and their store applications</p>
        </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-gradient-to-br from-indigo-100 to-indigo-200 rounded-full flex items-center justify-center text-indigo-600 font-semibold text-sm shadow-sm">
                        {(seller.storeName || seller.shopName || 'S').charAt(0).toUpperCase()}
                      </div>
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          {seller.ownerName || `${seller.firstName || ''} ${seller.lastName || ''}` || 'N/A'}
                        </p>
                      </div>
                    </div>
                  </td>
      ...
      <ConfirmModal
        ...
        title={
          modalAction === 'approve' ? 'Approve Seller' : 
          ...
          modalAction === 'approve'
            ...
            : (
              <div className="text-left space-y-2">
                <p><strong>Store:</strong> {selectedSeller?.storeName || selectedSeller?.shopName}</p>
                <p><strong>Owner:</strong> {selectedSeller?.ownerName || `${selectedSeller?.firstName} ${selectedSeller?.lastName}`}</p>
                <p><strong>Email:</strong> {selectedSeller?.email}</p>
                <p><strong>Phone:</strong> {selectedSeller?.phone || 'N/A'}</p>
                <p><strong>Status:</strong> {selectedSeller?.status || 'PENDING'}</p>
              </div>
            )
        }
        confirmText={modalAction === 'approve' ? 'Approve Seller' : modalAction === 'reject' ? 'Reject Seller' : 'Close'}
        type={modalAction === 'approve' ? 'success' : modalAction === 'reject' ? 'danger' : 'info'}
      />
    </div>
  );
};
